...
AUTOHOOK(SetStringValEngine, engine.dll + 0x416C20, void, (ConVar* var, const char* value))
{
	if (var->IsFlagSet(FCVAR_CROUTON)) 
		strcpy_s((char*)var->m_pszDefaultValue, (int)var->m_fMaxVal, value);
	else
		SetStringValEngine(var, value);
}
...
ON_DLL_LOAD("engine.dll", ConVar, (CModule module))
{
	...
	g_soupDLLIdentifier = R2::g_pCVar->AllocateDLLIdentifier();
}

ConVar* RegisterConVar(const char* name, const char* defaultVal, int flags, const char* help)
{
	ConVar* pVar = new ConVar(name, defaultVal, flags, help);
	// stamp our DLL identifier into ConCommandBase at +0x14 (the unk int32 in pad_0011[7])
	*reinterpret_cast<int*>(reinterpret_cast<char*>(&pVar->m_ConCommandBase) + 0x14) = g_soupDLLIdentifier;
	s_registeredConVars.push_back(pVar);
	return pVar;
}
...
void CleanupConVars()
{
	if (R2::g_pCVar)
	{
		for (ConVar* p : s_registeredConVars)
		{
			R2::g_pCVar->UnregisterConCommand(&p->m_ConCommandBase);
			delete p;
		}
	}
...
{
	sv_soup_test = RegisterConVar("sv_soup_test", "0", FCVAR_NONE, "Soup test convar");
}
...
{
	spdlog::info("Registering SoupVar \"{}\"", pszName);
...
ConVar::ConVar(const char* pszName, const char* stringPtr, int bufferSize, int nFlags, const char* pszHelpString)
{
	assert(nFlags & FCVAR_CROUTON);
	spdlog::info("Registering crouton var \"{}\"", pszName);
	this->m_ConCommandBase.m_pConCommandBaseVTable = g_pConVar_Vtable;
	this->m_ConCommandBase.s_pConCommandBases = (ConCommandBase*)g_pIConVar_Vtable;

	conVarMalloc(&this->m_pMalloc, 0, 0); // Allocate new memory for ConVar.
	conVarRegister(this, pszName, stringPtr, nFlags, pszHelpString, 0, 0, 0, 0, 0);
	this->m_Value.m_pszString = stringPtr;
	this->m_bHasMax = false;
	this->m_fMaxVal = bufferSize;
}
...
void ConVar::SetValue(const char* pszValue)
{
	...
	if (!SetColorFromString(pszValue))
	{
		...
		{
			spdlog::info("Warning: SoupVar '{}' = '{}' is infinite, clamping value.\n", GetBaseName(), pszValue);
			flNewValue = FLT_MAX;
		}
