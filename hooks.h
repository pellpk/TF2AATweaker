...
{
	static const bool bDedicated = !!strstr(GetCommandLineA(), "-dedicated");
	return bDedicated;
}
...
public:
	...
	{
		SIGNATURE, // we're using a signature and a module name
...
public:
		...
		: pHookFunc(func), ppOrigFunc(orig)
	{
		if (strstr(addrString, "@"))
			iAddressResolutionMode = SIGNATURE;
		else
	...
	void Dispatch()
	{
		...
		case SIGNATURE:
		{
			...
			if (!pModuleAddr)
				break;
			// get the CModule
			CModule* pModule = new CModule(pModuleAddr);
			...
			// seek until we hit the start of the pattern
			for (; !(pAddrString[iOffsetBegin] == '"') && pAddrString[iOffsetBegin]; iOffsetBegin++)
				;
			iOffsetBegin++; // get past the "

			int iOffsetEnd = iOffsetBegin;
			// now seek again until we reach the end of the pattern
			for (; !(pAddrString[iOffsetEnd] == '"') && pAddrString[iOffsetEnd]; iOffsetEnd++)
				;
			targetAddr = (LPVOID)(pModule->FindPattern((std::string(pAddrString).substr(iOffsetBegin, iOffsetEnd-iOffsetBegin).c_str())));
			break;
		}
		case OFFSET_STRING:
		{
		...
		if (MH_CreateHook(targetAddr, pHookFunc, ppOrigFunc) == MH_OK)
		{
			if (MH_EnableHook(targetAddr) == MH_OK)
				...
				spdlog::info("MH_EnableHook failed for function {}", pFuncName);
		}
		else
			spdlog::info("MH_CreateHook failed for function {}", pFuncName);
	}
};

// hook a function at a given offset from a dll to be dispatched with AUTOHOOK_DISPATCH()
#define AUTOHOOK(name, addrString, type, args)                                                                          \
